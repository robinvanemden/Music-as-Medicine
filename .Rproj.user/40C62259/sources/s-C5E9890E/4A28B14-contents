...
#'
#' @export
qrs_detect <- function(df, samplingrate, manualthr = 15, interpolate = TRUE, invert_polarity = FALSE) {
  
  ecg <- as.vector(df$heartrate)
  idx <- as.vector(df$idx)
  ...
  # ecg.new <- moving_average(ecg.new, n = 15)
  #ecg.new <- moving_average(ecg.new, n = 31)
  ...
  r.peaks[,1] <- idx[r.peaks[,1]]
  
  df$is_peak[idx %in% r.peaks[,1]] <- TRUE
  
  df$heartrate <- as.vector(df$heartrate)
  
  df[is_peak == TRUE & heartrate < manualthr]$is_peak <- FALSE
